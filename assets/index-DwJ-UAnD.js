...
`;d(document.querySelector("#counter"));const i=new Worker("./check.js");i.onmessage=r=>{r.data.type==="hasUpdate"&&alert("版本已更新")};const p=document.getElementById("get-update"),g=document.getElementById("dis-worker");g.addEventListener("click",function(){i.terminate()});p.addEventListener("click",async function(){const o=(await fetch(`${u}m.json?v=${Date.now()}`)).headers.get("etag");i.postMessage({type:"check",etag:o})});
