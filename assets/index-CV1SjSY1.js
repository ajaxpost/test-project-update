...
`;d(document.querySelector("#counter"));const i=new Worker("./check.js");i.onmessage=r=>{r.data.type==="hasUpdate"&&alert("版本已更新")};const u=document.getElementById("get-update"),p=document.getElementById("dis-worker");p.addEventListener("click",function(){i.terminate()});u.addEventListener("click",async function(){const o=(await fetch(`/m.json?v=${Date.now()}`)).headers.get("etag");i.postMessage({type:"check",etag:o})});
