...
`;d(document.querySelector("#counter"));const i=new u("./check.js");i.onmessage=o=>{o.data.type==="hasUpdate"&&alert("版本已更新")};const g=document.getElementById("get-update"),f=document.getElementById("dis-worker");f.addEventListener("click",function(){i.terminate()});g.addEventListener("click",async function(){const r=(await fetch(`${p}m.json?v=${Date.now()}`)).headers.get("etag");i.postMessage({type:"check",etag:r})});
